...
bool isCamelCase(String word) {
  ...
  var first_test =
      _isCapitalChar(letters[0]) ? _isCapitalChar : _isNonCapitalChar;
  var second_test =
      first_test == _isCapitalChar ? _isNonCapitalChar : _isCapitalChar;
  ...
  if (first_test(letters[0])) {
    ...
    if (idx < letters.length - 1) {
      if (letters.indexWhere((l) => first_test(l), idx) != -1) return true;
    }
    return false;
  } else {
    return false;
...
class Behaviors {
  ...
  static Iterable<String> deletes(Iterable<List<String>> _splits) {
    return _splits
        .where((sector) => sector[1].isNotEmpty)
        .map((sector) => sector[0] + sector[1].substring(1));
  }

  static Iterable<String> transposes(Iterable<List<String>> _splits) {
    return _splits.where((sector) => sector[1].length > 1).map((sector) =>
        sector[0] + sector[1][1] + sector[1][0] + sector[1].substring(2));
  }

  static List<String> replaces(Iterable<List<String>> _splits) {
    var s = _splits.where((sector) => sector[1].isNotEmpty).toList();
    return _LETTERS.fold([], (all, a) {
      return all +
          s.map((sector) => sector[0] + a + sector[1].substring(1)).toList();
    });
  }

  static List<String> inserts(Iterable<List<String>> _splits) {
    var s = _splits.where((sector) => sector[1].isNotEmpty).toList();
    return _LETTERS.fold([], (all, a) {
...
class Spell {
  ...
  bool useCache;
  bool preRender;
  late Set<String> dict;
  late Set<String> split_dict;
  late Map<String, Set<String>> typo_split_dict;
  final Map<String, Set<String>> CACHE = {};
  late bool Function(String db, String type) matcher;

  factory Spell({
      required Set<String> dict,
      bool camelCase = false,
      bool useCache = false,
      bool preRender = true,
      TSpellMatcher? matcher
  }) {
    return Spell.init(
        dict: dict,
        camelCase: camelCase,
        useCache: useCache,
        preRender: preRender,
        matcher: matcher ?? (db, typo) => db == typo);
  }
  ...
  Spell.init({
    ...
    this.split_dict = camelCase
      ? dict.map((x) => CamelCaseTyping(x).words)
        .expand((list) => list)
        .toSet()
      : dict;
    if (preRender)
      this.typo_split_dict = Map<String, Set<String>>.fromIterable(
          split_dict,
          key: (d) => d,
          value: (d) => getWordTypos(d));
    else
      throw Exception("Not Implemented yet");
  }

  Iterable<String> _correctTypingsViaPreRenderedTypoDict(String typing) {
    return typo_split_dict.entries
        .where((typos) => typos.value.any((v) => matcher(v, typing)))
        .map((typos) => typos.key);
  }

  Iterable<String> _correctTypingsViaUserTyposSearchInKnown(String typing) {
  ...
  Iterable<String> _camelCaseCorrection(String typing) {
    var cc = CamelCaseTyping(typing);
    if (preRender) {
      if (dict.contains(typing)) return CamelCaseTyping(typing).words;
      return cc.words
          .expand((word) => _correctTypingsViaPreRenderedTypoDict(word));
    }
    return cc.words
        .expand((word) => _correctTypingsViaUserTyposSearchInKnown(word));
  }

  Iterable<String> _generalCorrection(String typing) {
    if (preRender) return _correctTypingsViaPreRenderedTypoDict(typing);
    return _correctTypingsViaUserTyposSearchInKnown(typing);
  }

  Iterable<String>? filterWordsByKnown(Set<String> typings) {
    var ret = typings.where((typing) => dict.any((db) => matcher(db, typing)));
    return ret.length > 0 ? ret : null;
  }

  Iterable<String> correct(String typing) {
    if (camelCase)
      return _camelCaseCorrection(typing);
    return _generalCorrection(typing);
  }

  Set<String> getWordTypos(String word, [int distance = 1]) {
    if (distance == 1) {
      if (useCache && CACHE.containsKey(word))
        return CACHE[word]!;
  ...
  static Set<String> wordTypos(String word) {
    var possibilities = Set<String>();
    var splits = Behaviors.splits(word).toList();
    possibilities = (Behaviors.deletes(splits).toList() +
...
abstract class TypingParser{
  abstract List<int> pos;
  abstract List<String> words;
}
...
/*
   ...
   [EX]
      ...
      cc.pos   == [0,5]
*/
class CamelCaseTyping implements TypingParser{
  static Map<String, CamelCaseTyping> _instances = {};
  late List<int> pos;
  late List<String> words;

  factory CamelCaseTyping(String typing) {
    if (_instances.containsKey(typing))
      return _instances[typing]!;
